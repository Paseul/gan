...
if __name__ == '__main__':
    ...
    #stage2_dis = multi_gpu_model(stage2_dis, gpus=2)
    stage2_dis.compile(loss='binary_crossentropy', optimizer=dis_optimizer)

    stage1_gen = build_stage1_generator()
    #stage1_gen = multi_gpu_model(stage1_gen, gpus=2)
    ...
    #stage2_gen = multi_gpu_model(stage2_gen, gpus=2)
    stage2_gen.compile(loss="binary_crossentropy", optimizer=gen_optimizer)

    embedding_compressor_model = build_embedding_compressor_model()
    #embedding_compressor_model = multi_gpu_model(embedding_compressor_model, gpus=2)
    embedding_compressor_model.compile(loss='binary_crossentropy', optimizer='adam')

    adversarial_model = build_adversarial_model(stage2_gen, stage2_dis, stage1_gen)
    #adversarial_model = multi_gpu_model(adversarial_model, gpus=2)
    adversarial_model.compile(loss=['binary_crossentropy', KL_loss], loss_weights=[1.0, 2.0],
                              optimizer=gen_optimizer, metrics=None)

    current_time = datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
    dis2_log_dir = 'logs/gradient_tape/' + current_time + '/dis2_loss'
    gen2_log_dir = 'logs/gradient_tape/' + current_time + '/gen2_loss'
    dis2_summary_writer = tf.summary.create_file_writer(dis2_log_dir)
    gen2_summary_writer = tf.summary.create_file_writer(gen2_log_dir)
    ...
    for epoch in range(epochs):
        ...
        """
        with dis2_summary_writer.as_default():
            tf.summary.scalar('loss', np.mean(dis_losses), step=epoch)
        with gen2_summary_writer.as_default():
            tf.summary.scalar('loss', np.mean(gen_losses[0]), step=epoch)
